...
Commands:
  (none)  将每个 CBOR 项显示为美化诊断记法（§8）
  decode  将 CBOR 项转换为美化的 JSON，或用 --diag
          转换为美化的诊断记法
  encode  将 JSON 值转换为 CBOR 项；加 --diag 时读取 CDN 文本
  validate
          校验一个或多个完整 CBOR 项
```
...
| 编码测试夹具     | `cbor encode` 将 JSON 值转换为 CBOR 字节；`cbor encode --diag` 读取 Concise Diagnostic Notation。          |
| 安全复制字节     | `cbor encode --hex` 输出可复制的小写十六进制；可与 `--diag` 组合生成 CDN 测试夹具。                        |
| 处理序列         | 多个 JSON 或 CDN 值会成为一个 CBOR 序列；CBOR 序列逐项解码。                                                |
...
文件路径。一切都是流式的：多个 JSON 或 CDN 值会成为一个 CBOR 序列（RFC 8742），
...
评论或另一个 `cbor` 调用里时，使用 `cbor encode --hex`。当测试夹具用 CDN
比 JSON 更容易书写时，使用 `cbor encode --diag --hex`。
...
RFC 8949 附录 A 完全一致。超大的 bignum payload 会退回为显式 tag/bytes
记法，以保持渲染有界。
...
`cbor decode` 将每一项美化输出为 JSON，或用 `-d`/`--diag` 输出为缩进的诊断记法。
诊断路径读取原始项字节，因此会保留不定长和其他传输细节；JSON 路径会通过
`Value` 解码，因此使用 JSON 兼容的拼写。
...
项。加上 `--diag` 可改为读取 Concise Diagnostic Notation；加上 `--hex` 可输出
可复制的小写十六进制文本：
